...
Apart from four gene duplications (marked δ1, δ2, δ3 and δ4), several gene losses too would need to be assumed to explain the pattern in the gene tree in Fig. 2.7.
...
Tree 1, 2 and 3 are incongruent (i.e. they contain clades that contradict those in the other trees), therefore it is important to apply the right consensus approach in order to visualise the differences between the trees. 
...
The most simple method for character-based tree building is parsimony analysis in which, character-by-character, the fit (of each character) onto a candidate tree is counted (see Fig 4.3). 
...
When distances are additive, the additive tree perfectly describes them (Fig 4.3b). However, here sequences b and c have the smallest distance (d(b,c)=3), but are not most closely related. 
:::
...
NJ is implemented in MEGA11 used in the practical.
...
Subsequently, a candidate tree is considered and the likelihood LD of observing the data (the MSA) is calculated given that model and that particular tree. 
...
The likelihood LD of observing the data (your MSA) is calculated again and this time the likelihood may actually be better. 
More trees are evaluated and more model parameter values are considered, all the time keeping track of LD until no further increase LD can be obtained. 
